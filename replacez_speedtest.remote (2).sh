...
validate_interval_hours() {
    ...
    [[ "$interval_hours" =~ ^[0-9]+$ ]] && [ "$interval_hours" -gt 0 ]
}
...
EOF
    ...
    cat > "$TIMER_FILE" <<EOF
[Unit]
Description=Run replacez speedtest script every ${interval_hours} hours
...
# FixedIntervalHours=${interval_hours}
...
EOF
    ...
    echo "Initial run: triggered once immediately"
    echo "Run interval: every ${interval_hours} hours"
    echo "Next scheduled run: ${interval_hours} hours from now"
...
usage() {
    cat <<EOF
Usage:
  bash $0 --install-systemd [--interval-hours N]
...
Examples:
  bash $0 --install-systemd --interval-hours 6
  bash $0 --install-systemd --interval-hours 12
EOF
}
...
case "${1:-}" in
    --install-systemd)
        ...
        if [ "$#" -ne 0 ] || ! validate_interval_hours "$interval_hours"; then
            echo "Invalid interval. Please provide a positive integer hour value."
            usage
            exit 1
        fi

        install_systemd_timer "$interval_hours"
        ;;
    --uninstall-systemd)
        uninstall_systemd_timer
        ;;
